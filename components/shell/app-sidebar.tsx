...
import { ChevronsUpDown, Eye, FolderKanban, KeyRound, LogOut, Menu, Users } from "lucide-react";
...
function NavContent({
  ...
  const yours = projects.filter((p) => p.involved);
  const others = projects.filter((p) => !p.involved);
  ...
  return (
    <div className={cn("flex flex-col", sheet ? "max-h-[calc(85svh-1.5rem)] min-h-0" : "h-full")}>
      ...
      {projects.length > 0 && (
        <div className={cn("min-h-0 flex-1 space-y-5 overflow-y-auto px-3", sheet ? "mt-1" : "mt-6")}>
          {[
            { label: others.length ? "Your projects" : "Projects", list: yours, readOnly: false },
            { label: "Other projects", list: others, readOnly: true },
          ]
            .filter((g) => g.list.length > 0)
            .map((group) => (
              <div key={group.label}>
                <p className="flex items-center gap-1.5 px-2 pb-2 text-[11px] font-medium uppercase tracking-wider text-muted-foreground/70">
                  {group.label}
                  {group.readOnly && (
                    <span className="inline-flex items-center gap-1 normal-case tracking-normal" title="View only">
                  ...
                  {group.list.map((p) => {
                    ...
                    return (
                      <Link
                        ...
                        onClick={onNavigate}
                        title={group.readOnly ? `${p.title} (view only)` : p.title}
                        className={cn(
                          ...
                          group.readOnly && !active && "opacity-80"
                        )}
                      >
                        {active && (
                          <motion.span
                        ...
                        />
                        <span className="relative flex-1 truncate">{p.title}</span>
                        {group.readOnly && <Eye className="relative size-3.5 shrink-0 text-muted-foreground/60" />}
                      </Link>
                    );
                  })}
                </div>
